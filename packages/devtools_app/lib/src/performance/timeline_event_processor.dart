...
bool hideMessageLoopFlushTaskEvents = true;
...
//        .toList();
    ...
    final _traceEvents = (traceEvents.where((event) {
      ...
      final shouldProcess = ts != null &&
          !(hideMessageLoopFlushTaskEvents && isMessageLoopFlushTasks);
    ...
    while (_traceEventsProcessed < _traceEvents.length) {
      ...
      // Divide by 2 because once this finishes, we'll consider the progress to
      // be at 50%. After we process the events we need to do the work of
      // putting them in their proper event groups in [TimelineController].
    ...
    if (timelineController.timelineEvents.value.isNotEmpty) {
      ...
      // [timelineController].
      timelineController.time.end =
          Duration(microseconds: timelineController.endTimestampMicros);
    } else {
  ...
  void _processBatch(int batchSize, List<TraceEventWrapper> traceEvents) {
    ...
    for (int i = _traceEventsProcessed; i < batchEnd; i++) {
      ...
      timelineController.traceEvents.add(eventWrapper);
